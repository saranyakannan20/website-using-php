...
if (isset($_POST['apply_place'])) //??????????????//
{
  ...
  if($status=='NS')
  {
  $user_check_query = "SELECT * FROM companybranch inner join company on companybranch.COMPANY_NAME=company.COMPANY_NAME where company.APPROVAL='approved' and companybranch.C_TYPE='Placement' AND companybranch.BRANCH='$branch1' AND company.STATUS='visiting' and company.COMPANY_NAME='$company_name' LIMIT 1";
  $result = mysqli_query($db, $user_check_query);
  //The mysqli_fetch_assoc() function fetches a result row as an associative array.
  $user = mysqli_fetch_assoc($result);
  //$branch2=$user['BRANCH'];
  ...
  {
    array_push($errors, "Absent reached its maximum values"); 
  }
 

 
  $user_check_query = "SELECT * FROM registered_placements WHERE STUDENT_ID='$vari' AND COMPANY_NAME='$company_name' LIMIT 1";
  $result = mysqli_query($db, $user_check_query);
  //The mysqli_fetch_assoc() function fetches a result row as an associative array.
  $user = mysqli_fetch_assoc($result);
  ...
  {
    array_push($errors, "Already Placed"); 
  }
  ...
  if (count($errors) == 0) 
  {
    $query = "INSERT INTO registered_placements (STUDENT_ID,STUDENT_NAME,COMPANY_NAME)VALUES('$vari', '$student_name', '$company_name')";
...
<html>
<head>
  <title>student Apply for place</title>
...
<!--/*The PHP superglobals $_GET and $_POST are used to collect form-data.-->  
  ...
  <form method="post" action="apply_place.php">
  <?php include('wrong.php'); ?>
  <?php include('positive.php'); ?>
 <div class="input-group">
      <label>Company For Placement</label>
...
<option>Company Name</option>
      ...
      $branch_st=$user['BRANCH'];
       $user_check_query1 = " SELECT DISTINCT company.COMPANY_NAME FROM companybranch inner join company on companybranch.COMPANY_NAME=company.COMPANY_NAME where companybranch.C_TYPE='Placement' and company.STATUS='visiting' and company.APPROVAL='approved' and companybranch.BRANCH='$branch_st'";
    ...
    <div class="input-group">
      <button type="submit" class="btn" name="apply_place">Apply</button>
    </div>
    <p>
      Applied for the company? <a href="index_student_placement.php">Go Back</a>
    </p> 
  </form>
</body>
</html>